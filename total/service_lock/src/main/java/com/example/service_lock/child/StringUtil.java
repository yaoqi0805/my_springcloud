...
public class StringUtil {
	...
	public static String parseSql(String sql) {
		...
		String[] substring= sql.split(end);//用end将string切分为子串
		for(int i=0;i<substring.length-1;i++)
		{	
			ressub=replace(substring[i],start,replace);//替换case when为when
			ressub="!CASE "+ressub+" !END! ";
			
			StringBuffer sbprocess=new StringBuffer(ressub);
			sbprocess.deleteCharAt(6);
			ressub=sbprocess.toString();
			sb.append(ressub);
	...
	public static String delZero(String src) {
		if (src.length() > 6 && src.endsWith("000")) {
			return delZero(src.substring(0, src.length() - 3));
		} else if (src.length() <= 6 && src.endsWith("00")) {
			return delZero(src.substring(0, src.length() - 2));
		} else {
			return src;
		}
	}
	 ...
	 * @return
	 */
	public static int getChildrenLength(int parentLength) {
		int length;
		if(parentLength == 4){
			length = 6;
